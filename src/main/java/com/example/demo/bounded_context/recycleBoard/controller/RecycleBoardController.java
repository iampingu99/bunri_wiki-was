...
public class RecycleBoardController {
    ...
    @Operation(summary = "나눔 게시글 검색", description = "/paging?page=번호(1~),{option1}1-최신순,2-조회순 // {option2}1-제목포함검색, 2-닉네임검색")
    public ResponseEntity<?> searchPage(@PageableDefault(page = 1) Pageable pageable,
        ...
        Page<PageRecycleBoardDto> boardPages;

        if(option2==1)
            boardPages = recycleBoardService.titleSearch(pageable,option1,keyword);
        else
            boardPages = recycleBoardService.nickNameSearch(pageable,option1,keyword);
